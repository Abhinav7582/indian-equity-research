...
| 5 | Allocation system | **TOOLING COMPLETE 2026-08-30 — the policy itself is not, and cannot be by this project.** Three of four components built: the **belief checker** (A13, `research/beliefs.py`), the **pre-trade calculator** (`backtest/pretrade.py`) and the **drift monitor** (`backtest/drift.py`). The fourth — the declared target weights — is the owner's to set; `configs/target_allocation.yaml` ships with every weight `null` and the monitor **refuses a template** rather than shipping an allocation nobody chose. **First belief checked, B1:** mid/small-cap outperformance came back **NOT CONFIRMED** — the A13 second window split the archive at 2016 and found Midcap 150 beat the Nifty 100 in **46%** of 12-month windows before that date and **73%** after, with Smallcap 250 winning **one 5-year window in five** pre-2016 (median −14.5%). Every favourable full-sample figure is an average of two opposed regimes. **First thing the pre-trade calculator showed:** on a ₹5,00,000 position with a ₹1,00,000 gain, charges are ₹566 and tax is **₹19,887** — switching costs **4.21%** short-term against **0.24%** long-term, an eighteen-fold difference decided by the calendar alone. **COMPLETE 2026-09-06** with the fourth component, the **contribution router** (`route_contribution`, `scripts/plan_contribution.py`): new money split across whatever sits below target, proportional to the rupee shortfall, **nothing sold** so no capital gains and no exit charges. Run against the declared 25/24/50/1 it sends the owner's equity SIP to **debt and the US/crypto bucket, and equity gets ₹0** — the target says 25% and the book is at 26.0%. The tool is right; the policy disagrees with the owner's stated intent, and that one number is the only thing left in Phase 5 |
| 6 | Unexplored instruments research | **STARTED 2026-09-06 — 1 of 6 instruments done.** The '₹31.9 lakh at 6.6%' framing used earlier was **wrong and is corrected** in `docs/phase6_arbitrage_vs_fd.md`: PPF at 7.1% tax-free and EPF at 8%+ tax-free are the best risk-free rates in the book and both are locked. The genuinely addressable money is **₹14.54L of FDs plus ₹99,531 of savings**, not ₹31.9L. Arbitrage funds analysed first, on B2's promotion |
| 7 | Paper trading, then the A6 decision | **Dormant by design.** Gated on a strategy passing development; none has |

**958 tests · ruff clean · mypy strict clean · 12/12 mutation bugs caught.**
...
| **Arbitrage funds** — **DONE 2026-09-06**, `docs/phase6_arbitrage_vs_fd.md` | Equity taxation on a debt-like risk profile. At a 30% slab held past 12 months, a **category-average 6.22% fund needs an FD paying 7.78% to match it** — and the best FD in the market is ~7.25%. Worth **₹23,264/yr** on the ₹14.54L FD pot. Three caveats found: the ₹1.25L exemption is shared across all equity and only helps if realised annually; the yield is not contractual and breaks even against a 6.6% FD at ~5.28% gross; it is not capital-protected. Also **structurally more liquid than an FD** — T+1/T+2 with no penalty — which bears on open item 5 |
